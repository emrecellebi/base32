...
int main(int argc, char** argv)
{
	...
	if(argc <= 1)
	{
		printf("b32 : No command specified. Use 'b32 -h or --help' for a detailed command list");
		return 1;
	}
	...
	if(argc == 3 && (strcmp(argv[1], "-e") == 0 || strcmp(argv[1], "--encode") == 0))
	{
		...
		size_t enc_sz = base32_encode_alloc(data, inlen, &encode);
		printf("%s", encode);
	...
	if(argc == 3 && (strcmp(argv[1], "-d") == 0 || strcmp(argv[1], "--decode") == 0))
	{
		...
		base32_decode_alloc_ctx(&ctx, data, inlen, &decode, &inlen);
		printf("%s", decode);
	...
	if(argc == 2 && (!strcmp(argv[1], "-h") || !strcmp(argv[1], "--help")))
	{
		...
		printf("Usage: b32 [-options] <parameters>\n");
		printf("  -d, --decode <str>\n");
		printf("  -e, --encode <str>\n");
		printf("  -v, --version\n");
		printf("  -h, --help\n");
...
static void do_encode(FILE* in, FILE* out, uintmax_t wrap_column)
{
	...
	char outbuf[BASE32_LENGTH(ENC_BLOCKSIZE)];
	size_t sum;
	...
	do
	{
		...
		if(sum > 0)
		{
			base32_encode(inbuf, sum, outbuf, BASE32_LENGTH(sum));
			wrap_write(outbuf, BASE32_LENGTH(sum), wrap_column, &current_column, out);
		}
	}while(!feof(in) && !ferror(in) && sum == ENC_BLOCKSIZE);
}
...
static void do_decode(FILE* in, FILE* out, bool ignore_garbage)
{
	char inbuf[BASE32_LENGTH(DEC_BLOCKSIZE)];
	...
	do
	{
		...
		do
		{
			n = fread(inbuf + sum, 1, BASE32_LENGTH(DEC_BLOCKSIZE) - sum, in);
			if(ignore_garbage)
			{
		...
		}while(sum < BASE32_LENGTH(DEC_BLOCKSIZE) && !feof(in));
